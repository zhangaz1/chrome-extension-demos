...
Mappings.actions = {
    ...
    openLastLinkInTab: function(repeats) {
        RUNTIME('openLastLinkInTab', {
            repeats: repeats
        });
    },
    openNextLinkInTab: function(repeats) {
        RUNTIME('openNextLinkInTab', {
            repeats: repeats
        });
    },
    ...
    goToMark: function() {
        ...
        if (Scroll.positions.hasOwnProperty(key)) {
            Scroll.lastPosition = [document.scrollingElement.scrollLeft, document.scrollingElement.scrollTop];
    ...
    setMark: function() {
        Scroll.positions[Mappings.lastCommand.queue.slice(-1)] = [document.scrollingElement.scrollLeft, document.scrollingElement.scrollTop];
    },
    createHint: function() {
        Hints.create();
    },
